...
export default function MemberFilterBox() {
        ...
        <div className={style.search_wrap}>
            <div className={style.search_type}>
                <div className={style.radiobox_wrap}>
                    <input id="product0" type="radio" name="product_type" value="" checked={filter==="" ? true : false} onChange={(e) => { setFilter(e.target.value); }} />
                    <label htmlFor="product0" className={style.radio_label}>통합</label>
                </div>
                <div className={style.radiobox_wrap}>
                    <input id="product1" type="radio" name="product_type" value="이실장" checked={filter==="이실장" ? true : false}  onChange={(e) => { setFilter(e.target.value); }} />
                    <label htmlFor="product1" className={style.radio_label}>이실장</label>
                </div>
                <div className={style.radiobox_wrap}>
                    <input id="product2" type="radio" name="product_type" value="포커스" checked={filter==="포커스" ? true : false} onChange={(e) => { setFilter(e.target.value); }} />
                    <label htmlFor="product2" className={style.radio_label}>포커스</label>
                </div>
                <div className={style.radiobox_wrap}>
                    <input id="product3" type="radio" name="product_type" value="프리미엄" checked={filter==="프리미엄" ? true : false} onChange={(e) => { setFilter(e.target.value); }} />
