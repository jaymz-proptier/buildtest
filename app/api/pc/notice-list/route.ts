...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        const sql = `select bnSeq, noticeGubun, title, viewCount, DATE_FORMAT(regDate, '%y.%m.%d') as regDate from tb_board_notice where useYn = 'Y' order by (case when topYn = 'Y' then 0 else 99 end), regDate desc, title desc limit ?, 10`;
