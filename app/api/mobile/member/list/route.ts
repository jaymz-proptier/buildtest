...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        const sql = `select (${Number(searchParams.get("page") && searchParams.get("page")!=="0" ? searchParams.get("page") : 1)} + 1) as page, memSeq, 회원번호, 상품유형, 상호명, 대표자명, 휴대폰, (case when 소재지 > '' then 소재지 else concat(시도, ' ', (case when 시군구 = '세종시' then '' else 시군구 end), ' ',  읍면동, ' ', 상세주소) end) as 주소, 시도, 시군구, 읍면동, 상품명, 계약구분, 결제일, DATE_FORMAT(시작일, '%y.%m.%d') as 시작일, DATE_FORMAT(종료일, '%y.%m.%d') as 종료일, 상태, 계약전송수, 전송수, 계약단지명 from tb_data_member where useYn = 'Y'${sqlWhere} order by ${sqlOrderBy} limit ?, 10`;
